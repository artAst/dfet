...
CREATE TABLE pi_entry (entryId INTEGER PRIMARY KEY, entryKey VARCHAR(20), entryType INTEGER, status INTEGER, peopleId INTEGER, judgeNum VARCHAR(20), subSeqId INTEGER, heatId INTEGER, subHeatId INTEGER, FOREIGN KEY(subHeatId) REFERENCES pi_sub_heat(subHeatId) ON DELETE CASCADE);
CREATE TABLE pi_couple (coupleKey VARCHAR(20) PRIMARY KEY, coupleId INTEGER, uploadId INTEGER, entryKey VARCHAR(20), booked INTEGER, scratched INTEGER, danced INTEGER, future INTEGER, total INTEGER);
