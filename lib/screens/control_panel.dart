...
class _control_panelState extends State<control_panel> {
  ...
  void initState() {
    ...
    setState(() {
      ...
      eventDate.text =
          EventConfig.eventDate != null ? EventConfig.eventDate : "";
      ...
      eventTime.text = EventConfig.eventTime != null
          ? EventConfig.eventTime.toUpperCase()
          : "";
  ...
  void _saveDevice() {
    ...
    if (deviceNum.text.isNotEmpty) {
      ...
      isError = true;
      ScreenUtil.showMainFrameDialog(
          context, "Invalid", "Please Fill in Device Number");
    }
  ...
  void saveGlobal2() {
    ...
    if (eventName.text.isEmpty) {
      ScreenUtil.showMainFrameDialog(
          context, "Invalid", "Please Fill in Event Name");
      saveFlag = false;
    } else {
    ...
    if (eventDate.text.isEmpty) {
      ScreenUtil.showMainFrameDialog(
          context, "Invalid", "Please Fill in Event Date");
      saveFlag = false;
    } else {
    ...
    if (eventTime.text.isEmpty) {
      ScreenUtil.showMainFrameDialog(
          context, "Invalid", "Please Fill in Event Time");
      saveFlag = false;
    } else {
    ...
    if (saveFlag) {
      ...
      LoadContent.saveEventConfig(context).then((val) {
        MainFrameLoadingIndicator.hideLoading(context);
        ScreenUtil.showMainFrameDialog(
                context, "Save Success", "Event info saved. Press OK.")
            .then((val) {
          setState(() {
            EventConfig.eventName = eventName.text;
          });
...
// context, "Invalid", "Please Fill in " + profileTypes[i]['jobType'] + " Field");
              context,
              "Invalid",
              "Please Fill up all the missing Field/s.");
          saveGlobal3 = false;
          break;
        } else {
          saveGlobal3 = true;
    ...
    if (saveGlobal3) {
      ...
      for (var i = 0; i < profileTypes.length; i++) {
        //local save
        Preferences.setSharedValue(
            profileTypes[i]['jobType'],
            "enabled:" +
                profileTypes[i]['enabled'].toString() +
                ",timeoutVal:" +
                profileTypes[i]['timeoutVal']);
      ...
      LoadContent.saveTimeoutConfig(context).then((val) {
        ...
        if (val) {
          ScreenUtil.showMainFrameDialog(
                  context, "Save Success", "Timeout info saved. Press OK.")
              .then((val) {
            Navigator.maybePop(context);
            setState(() {
              _checkGlobal3isSaved();
          ...
          ScreenUtil.showMainFrameDialog(
                  context, "Save Failed", "Empty RPI IP'S")
              .then((val) {
            Navigator.maybePop(context);
          });
        }
  ...
  _checkGlobal3isSaved() async {
    for (var i = 0; i < profileTypes.length; i++) {
      print("----------------------------------------");
      String savedTypes =
          await Preferences.getSharedValue(profileTypes[i]['jobType']);
      if (savedTypes != null) {
                ...
                .map((k, v) => MapEntry(
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          Expanded(
                              child: Text(profileTypes[k]["jobType"].toString(),
                                  style: TextStyle(
                                      fontSize: 28.0,
                          ...
                          InkWell(
                            onTap: () {
                              setState(() {
                                profileTypes[k]["enabled"] = profileTypes[k]
                                            ["enabled"] ==
                                        true
                                    ? false
                                    : true; //change enable value of checkbox
                              });
                            },
                            child: Container(
                                width: 120.0,
                                child: Icon(
                                    profileTypes[k]["enabled"] ==
                                            true //change icon value of checkbox
                                        ? Icons.check_box
  ...
  _discardGlobal2() async {
    ...
    setState(() {
      ...
      if (EventConfig.screenTimeout && screenTimeouts.isEmpty) {
        ...
        if (!EventConfig.screenTimeout &&
            screenTimeouts.contains("sc_timeout")) {
          screenTimeouts.remove("sc_timeout");
        }
      }
                      ...
                      onPressed: () {
                        ScreenUtil.showMainFrameDialogWithCancel(
                          ...
                          if (val.toLowerCase() == "ok") {
                            ScreenUtil.showMainFrameDialog(
                                context, "Wipe Device Data", "You pressed OK");
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Padding(padding: EdgeInsets.only(top: 20.0)),
            Text(
                "Only use this when setting up a new event. All data on all tablets and all data on RPI will be deleted.",
            ...
            new DanceFrameButton(
              text: "RESET",
              onPressed: () {
                ScreenUtil.showWipeDialogWithCancel(context, "Wipe Global Data",
                        "Are you absolutely sure you want to wipe/reset everything for a new competition?")
                    .then((val) {
                  print("VALUE: $val");
                  if (val.toLowerCase() == "ok") {
                    ScreenUtil.showMainFrameDialog(context, "Wipe Success",
                            "Application will restart. Press ok to Exit")
                        .then((val) {
                      exit(0);
                    });
              ...
              new Container(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    new Image.asset("assets/images/Asset_43_4x.png",
                        height: 25.0)
                  ],
                ),
              )
